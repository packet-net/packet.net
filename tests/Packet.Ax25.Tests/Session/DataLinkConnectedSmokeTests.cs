...
public class DataLinkConnectedSmokeTests
{
    ...
    private static (Ax25Session session, RecordingActionDispatcher recorder, GuardEvaluator guards) NewSession(Guards g)
    {
    ...
    private static void AssertTransitionFires(Ax25Event evt, Guards guards)
    {
        
        var (s, r, ge) = NewSession(guards);
        var matching = DataLink_Connected.Transitions
            .Where(x => x.On == evt.Name)
            .Where(x => ge.Evaluate(x.Guard))
            .ToList();
        matching.Should().ContainSingle($"event '{evt.Name}' with the supplied guards should match exactly one transition in DataLink_Connected");
    ...
    [Fact] public void t01_dl_disconnect_request() =>
        AssertTransitionFires(new DlDisconnectRequest(), new Guards());
    ...
    [Fact] public void t18_dl_data_request() =>
        AssertTransitionFires(new DlDataRequest("x"u8.ToArray()), new Guards());

    // ─── Column 4 (I Frame Pops Off Queue) ─────────────────────────────
    [Fact] public void t19_i_frame_pops_off_queue_send_now_t1_running() =>
        AssertTransitionFires(new IFramePopsOffQueue(ReadOnlyMemory<byte>.Empty),
    ...
    [Fact] public void t23_dl_unit_data_request() =>
        AssertTransitionFires(new DlUnitDataRequest("x"u8.ToArray()), new Guards());

    // ─── Columns 6 & 7 (Flow control) ──────────────────────────────────
    [Fact] public void t24_dl_flow_off_when_not_busy() =>
        AssertTransitionFires(new DlFlowOffRequest(),
    ...
    [Fact] public void t26_dl_flow_on_when_busy_and_t1_not_running() =>
        AssertTransitionFires(new DlFlowOnRequest(),
    ...
    [Fact] public void t29_dl_connect_request_v22() =>
        AssertTransitionFires(new DlConnectRequest(), new Guards { Version22 = true });
    ...
    [Fact] public void t32_control_field_error_v22() =>
        AssertTransitionFires(new ControlFieldError(), new Guards { Version22 = true });

    [Fact] public void t33_control_field_error_v20() =>
        AssertTransitionFires(new ControlFieldError(), new Guards { Version22 = false });

    [Fact] public void t34_info_not_permitted_in_frame_v22() =>
        AssertTransitionFires(new InfoNotPermittedInFrame(), new Guards { Version22 = true });

    [Fact] public void t35_info_not_permitted_in_frame_v20() =>
        AssertTransitionFires(new InfoNotPermittedInFrame(), new Guards { Version22 = false });

    [Fact] public void t36_u_or_s_frame_length_error_v22() =>
        AssertTransitionFires(new UOrSFrameLengthError(), new Guards { Version22 = true });

    [Fact] public void t37_u_or_s_frame_length_error_v20() =>
        AssertTransitionFires(new UOrSFrameLengthError(), new Guards { Version22 = false });

    // ─── Columns 13-14 (Timer expiries) ────────────────────────────────
    [Fact] public void t38_t1_expiry() =>
        AssertTransitionFires(new T1Expiry(), new Guards());

    [Fact] public void t39_t3_expiry() =>
        AssertTransitionFires(new T3Expiry(), new Guards());

    // ─── Columns 15-16 (SABM/SABME collision) ──────────────────────────
    [Fact] public void t40_sabm_received_vs_neq_va() =>
        AssertTransitionFires(new SabmReceived(Frame()), new Guards { VsEqVa = false });

    [Fact] public void t41_sabm_received_vs_eq_va() =>
        AssertTransitionFires(new SabmReceived(Frame()), new Guards { VsEqVa = true });

    [Fact] public void t42_sabme_received_vs_neq_va() =>
        AssertTransitionFires(new SabmeReceived(Frame()), new Guards { VsEqVa = false });

    [Fact] public void t43_sabme_received_vs_eq_va() =>
        AssertTransitionFires(new SabmeReceived(Frame()), new Guards { VsEqVa = true });

    // ─── Columns 17-18 (FRMR/UA unexpected) ────────────────────────────
    [Fact] public void t44_frmr_received_v22() =>
        AssertTransitionFires(new FrmrReceived(Frame()), new Guards { Version22 = true });

    [Fact] public void t45_frmr_received_v20() =>
        AssertTransitionFires(new FrmrReceived(Frame()), new Guards { Version22 = false });

    [Fact] public void t46_ua_received_v22() =>
        AssertTransitionFires(new UaReceived(Frame()), new Guards { Version22 = true });

    [Fact] public void t47_ua_received_v20() =>
        AssertTransitionFires(new UaReceived(Frame()), new Guards { Version22 = false });

    // ─── Column 19 (UI) ────────────────────────────────────────────────
    [Fact] public void t48_ui_received_p_eq_0() =>
        AssertTransitionFires(new UiReceived(Frame()), new Guards { PEq1 = false });

    [Fact] public void t49_ui_received_p_eq_1() =>
        AssertTransitionFires(new UiReceived(Frame()), new Guards { PEq1 = true });

    // ─── Columns 20-21 (DISC/DM) ───────────────────────────────────────
    [Fact] public void t50_disc_received() =>
        AssertTransitionFires(new DiscReceived(Frame()), new Guards());

    [Fact] public void t51_dm_received() =>
        AssertTransitionFires(new DmReceived(Frame()), new Guards());

    // ─── Columns 22-23 (RR/RNR) ────────────────────────────────────────
    [Fact] public void t52_rr_received_nr_in_window() =>
        AssertTransitionFires(new RrReceived(Frame()), new Guards { NrInWindow = true });

    [Fact] public void t53_rr_received_nr_out_of_window_v22() =>
        AssertTransitionFires(new RrReceived(Frame()), new Guards { NrInWindow = false, Version22 = true });

    [Fact] public void t54_rr_received_nr_out_of_window_v20() =>
        AssertTransitionFires(new RrReceived(Frame()), new Guards { NrInWindow = false, Version22 = false });

    [Fact] public void t55_rnr_received_nr_in_window() =>
        AssertTransitionFires(new RnrReceived(Frame()), new Guards { NrInWindow = true });

    [Fact] public void t56_rnr_received_nr_out_of_window_v22() =>
        AssertTransitionFires(new RnrReceived(Frame()), new Guards { NrInWindow = false, Version22 = true });

    [Fact] public void t57_rnr_received_nr_out_of_window_v20() =>
        AssertTransitionFires(new RnrReceived(Frame()), new Guards { NrInWindow = false, Version22 = false });
    ...
    [Fact] public void t58_lm_seize_confirm_no_ack_pending()
    {
        ...
        AssertTransitionFires(new LmSeizeConfirm(), new Guards { AckPending = false });
    }

    [Fact] public void t59_lm_seize_confirm_ack_pending() =>
        AssertTransitionFires(new LmSeizeConfirm(), new Guards { AckPending = true });

    // ─── Columns 25-26 (SREJ/REJ) ──────────────────────────────────────
    [Fact] public void t60_srej_received_nr_in_window_pf_eq_0() =>
        AssertTransitionFires(new SrejReceived(Frame()),
    ...
    [Fact] public void t64_rej_received_nr_in_window() =>
        AssertTransitionFires(new RejReceived(Frame()), new Guards { NrInWindow = true });
