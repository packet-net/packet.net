...
/// Ax25Listener can run over AXUDP. Send puts the AX.25 frame body + the 2-octet
/// FCS into a datagram; receive (via <see cref="AxudpSocket"/>) strips + validates
/// the FCS and surfaces each datagram as a KISS Data frame. AXUDP always carries
/// the FCS — there is no FCS-less form.
/// </summary>
public sealed class AxudpKissModemTests
{
    ...
    {
        // AXUDP unconditionally carries the FCS, so the modem must put the 2-octet
        // FCS on the wire — that is how it talks to LinBPQ BPQAXIP / XRouter /
        // ax25ipd out of the box. (Survey verdict: every real AXIP/AXUDP peer
        // REQUIRES the FCS; the FCS-less form was self-only and removed — see
        // docs/strict-vs-pragmatic-audit.md.) Observe the raw wire with a plain
        // UdpClient (AxudpSocket would strip the FCS on receive).
    ...
    [Fact]
    public async Task ReadFrames_strips_the_fcs_and_surfaces_each_datagram_as_a_kiss_data_frame()
    {
        // A modem sender (FCS appended) → a modem receiver: the receive path strips +
        // validates the FCS and surfaces the bare AX.25 body as a KISS Data frame, so
        // the listener's parser (which rejects trailing bytes on S-frames) sees a
        // clean frame.
        await using var receiver = new AxudpKissModem(new IPEndPoint(IPAddress.Loopback, 1), localPort: 0);
        await using var sender = new AxudpKissModem(new IPEndPoint(IPAddress.Loopback, receiver.LocalPort), localPort: 0);
    ...
    {
        await using var modem = new AxudpKissModem(new IPEndPoint(IPAddress.Loopback, 1), localPort: 0);
