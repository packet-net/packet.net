...
**Latest amendment:** [§17 entry 2026-06-04 — Node kiss-tcp ports self-heal across a TNC/softmodem bounce: `ReconnectingKissModem` (backoff + KISS-param replay), so a dropped link reconnects instead of the port silently dying (#50)](#17-amendment-log)
...
- **Hot reconfiguration (marquee).** `NodeHostedService` (`BackgroundService`) is the sole `OnChange` subscriber; it computes a **field-level delta** (`ReconcilePlanner.Plan`, pure + property-tested) and a `PortSupervisor` (one `Ax25Listener` per port, keyed on `PortConfig.Id`) reconciles, touching only what changed: added/removed/enabled-toggled → bring up/tear down that port; **transport changed → single-port restart**; **KISS params changed → applied live** via the `IKissModem` setters (no restart); **AX.25 params changed → recorded for the next bring-up, live listener + sessions untouched** (the engine seeds `Ax25ListenerOptions` at construction and is consumed as-is, so re-seeding a running listener would mean rebuilding it = dropping sessions, which would violate "don't mutate live sessions"; documented design call — see [§17](#17-amendment-log)); **identity callsign changed → node-wide reset** (every listener recreated, logged as a warning); **telnet bind/port/enabled changed → restart the telnet listener only**; **services changed → reference swap** (the console reads `Services` live through the provider). Single-threaded reconcile (serialised + coalesced). A *runtime* fault bringing one port up (e.g. a serial device that won't open) faults only that port (logged); the rest of the reconcile completes and `Current` still advances. **Invariant (property-tested + integration-tested): a delta disrupts exactly the ports whose restart-class fields changed; hot-class changes restart nothing; sessions on untouched ports survive with the same object identity.**
