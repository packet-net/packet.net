...
**Follow-up — ax25-ts is still net-sim-only.** The TypeScript runtime (`@packet-net/ax25`, `m0lte/ax25-ts`) has no frame-perfect/AXUDP transport yet, so its protocol interop (the AX.25 `linbpq-via-netsim` round-trip + the NET/ROM NODES-ingest test, run by this repo's `interop.yml` sibling-clone step — phase B) remains on net-sim. A Tier-2-for-ax25-ts (an AXUDP/BPQAXIP transport in ax25-ts, then the same protocol re-home) is a future candidate, mirroring this PR.
