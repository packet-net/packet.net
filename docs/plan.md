...
**Latest amendment:** [§17 entry 2026-06-03 — v2.2 arc V5b: independent mod-128 interop vs direwolf (the real-peer leg; #177)](#17-amendment-log)
...
| 5.Z V4 🚫 | mod-128 **segmentation/reassembly** (> N1 payload) vs **direwolf** | **BLOCKED by a spec-figure divergence (not a defect in our segmenter).** Our `Segmenter` follows AX.25 v2.2 Figure 6.2 literally (info field = one F/X octet + payload, PID 0x08); Dire Wolf's first segment carries an **extra inner-PID octet** (`ax25_link.c` `dl_data_request` L1320–1336: `[F/X][original PID][payload]`) so its reassembler can recover the L3 PID. The §6.6 prose ("a two-octet header") admits both readings; we took Figure 6.2, Dire Wolf took inner-PID — wire-incompatible for segmentation (a 220-byte send arrives as 219 mangled bytes). `DirewolfMod128Interop.Segmentation_..._RoundTripBlockedByFirstSegmentPidDivergence` proves the **forward leg** (our 0x08 segment I-frames reach Dire Wolf's reassembler) then **Skips** the exact-payload round-trip with this divergence as the reason. Per repo policy the spec is canonical and Dire Wolf is an interop target — we don't bend our segmenter. (Candidate ax25spec clarification; not filed.) mod-128 segmentation proven in-process (`SegmentationIntegrationConformanceTests`). (V5b §17 2026-06-03) |
