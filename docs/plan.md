...
- **V5 — Conformance + interop + parity** (medium effort; packet.net + ax25-ts). Behavioural coverage of `AwaitingV22Connection` (15/25 since V2 — establishment column done; residual misses are never-produced error inputs + not-layer-3-initiated collision branches), mod-128 data transfer + loss recovery, the XID negotiation paths, and segmentation — via the harness in extended mode; lift the transition-coverage ledger accordingly. *(V4 delivered the in-process harness coverage of mod-128 REJ+SREJ loss recovery — incl. window-wrap — and wired segmentation round-trip + loss recovery; V5's remaining conformance scope is the `AwaitingV22Connection` ledger lift + the interop rows below + ax25-ts parity.)* **Interop: every v2.2 feature on the wire, not just connect** — the §7.1 matrix has the enumerated rows (SABME connect both directions, version-negotiation fallback, mod-128 windowed transfer, mod-128 REJ+SREJ loss recovery, segmentation/reassembly, XID negotiation) against **LinBPQ and direwolf** — both source-confirmed mod-128 stacks (see `[[ax25-mod128-peer-support]]`); **rax25** is the v2.0-only peer for the fallback test (its `sabme()` is a TODO), and XID is BPQ-only. Each feature lands its interop row as its V-leg completes (mod-128 transfer/loss/segmentation with V4, XID with V3) rather than all deferred to V5. Verify ax25-ts parity throughout (its own interop leg via `interop.yml`) and cut a coordinated release.
...
| Management Data-Link | C5.1, C5.2 (+ C5.3–C5.8 subroutines) | 2 | Ready, Negotiating | 🟡 **prose-bootstrap** (`verification_pending`) — figc5.1/figc5.2 hand-authored from §6.3.2 + App-C5.3 *prose* (Tom-directed exception; figc5.x never redrawn, PNGs all-black), shipped in `Packet.Ax25.Sdl` 0.9.0 and driven by `Ax25ManagementDataLink` (§5.Z V3 part 2, §17 2026-06-02). The figc5.3–5.8 per-parameter subroutines are collapsed to one `Apply Negotiated Parameters` placeholder; the *responder* XID-command path + error A are un-transcribed figure detail (runtime helper). figc5.x graphml redraw + figure-faithful transcription is the pending backfill. |
...
| 5.Z V2 ⬜ | SABME (mod-128) connect vs **LinBPQ** over net-sim AFSK1200 | our `DL-CONNECT` (prefer-v2.2) sends SABME; BPQ answers UA; both reach Connected with modulo 128; DISC/UA tears down. Both directions (we initiate / BPQ initiates) |
| 5.Z V2 ⬜ | SABME (mod-128) connect vs **direwolf** over net-sim AFSK1200 | as above vs direwolf (figure-faithful oracle; its source documents the SABME handshake) |
| 5.Z V2 ⬜ | Version-negotiation **fallback**: SABME → a v2.0-only peer | peer answers FRMR (or DM, §975) → we retry SABM and connect mod-8. Peer = **rax25** (no SABME — `fn sabme()` is TODO) or a BPQ port pinned v2.0 |
| 5.Z V4 ⬜ | mod-128 **windowed data transfer** (k>7, e.g. 16/32) vs **LinBPQ** + **direwolf** | bidirectional I-frames with 7-bit N(S)/N(R); peer log shows extended control + correct seq; payload round-trips; V(s)/V(a) converge |
| 5.Z V4 ⬜ | mod-128 **loss recovery** (REJ + SREJ) under 30% net-sim loss vs **LinBPQ** + **direwolf** | a > k-frame transfer with induced loss recovers and converges in the mod-128 sequence space; no FRMR |
| 5.Z V4 ⬜ | **Segmentation/reassembly** (> N1 payload) vs **LinBPQ** + **direwolf** | our segmenter splits a > N1 payload (PID 0x08 segment-control); peer reassembles + delivers intact; and the reverse (peer segments → we reassemble) |
| 5.Z V3 ⬜ | **XID negotiation** vs **LinBPQ** | XID command/response exchange agrees modulo / window k / SREJ / N1 / T1 / N2; both sides apply the negotiated values; the link then runs at the agreed modulo (not a forced flag) |
