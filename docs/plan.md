...
| 9 ✅ | **NET/ROM L3 origination + L4 circuits** vs **LinBPQ** over net-sim | ✅ `NetRomL4CircuitViaNetsim.Pdn_originates_NODES_and_runs_L4_circuits_both_ways_with_real_linbpq` — the transmitting NET/ROM stack validated against a **real LinBPQ 6.0.25.23**. **L3 both ways:** pdn (`netRom.broadcast`, alias `PNL4`) originates NODES → BPQ learns it (`NODES`→`PNL4:PNL4ND`, `ROUTES`→`3 PNL4ND 192 1`, queried via the sysop session), and pdn hears BPQ's `PNTST:PN0TST`. **L4 pdn→BPQ (core):** `ConnectCircuitAsync` opens an interlink AX.25 PID-0xCF session + originates an L4 circuit (`NetRomNodeConnection`, `TransportKind=NetRom`) → a command round-trips to BPQ's node prompt over the circuit (BPQ's `PN0TST` identity + CTEXT relay back — Information both ways) → clean Disconnect Request/Acknowledge. **L4 BPQ→pdn (reverse):** BPQ's `C PNL4` originates the circuit to pdn (`IncomingCircuit` fires with the correct originating user + window), pdn accepts + bridges, BPQ reports `Connected to PNL4:PNL4ND`, a relayed line round-trips. **BPQ framing divergence found + fixed (lab-deployable):** BPQ carries the Connect Request's proposed window + originating user/node in the **info field** (`[window][user][node]`), not the transport-header TX byte where we had the window — fixed via the new `ConnectRequestInfo` codec (the canonical appendix layout; spec-faithful default, not a BPQ-ism). `bpq32.cfg` needed no functional change (`SIMPLE=1` + `QUALITY=192` suffice). Multi-hop pdn→via-BPQ→XRouter not feasible (topology: a links to BPQ but BPQ↛XRouter). Deterministic (5/5 local). (§17 2026-06-05, PR #309) **Isolation:** this + the read-only NODES-ingest row are tagged `[Trait("Group", "NetRom")]` and run in their own clean-stack-fenced phase of `interop.yml`, isolated from the timing-sensitive AX.25 tests — see §7.2 (§17 2026-06-05). |
