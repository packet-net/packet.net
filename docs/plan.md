...
**Latest amendment:** [§17 entry 2026-06-09 — Candidate pattern captured (doc-only, not adopted): passkeys on a LAN-only node — [`docs/passkeys-lan-trust-pattern.md`](passkeys-lan-trust-pattern.md). Answers the open question the TLS part parked — a *trusted* secure context for WebAuthn on a node distributed to strangers' LANs, without a per-device root-CA install. Turns on two hard browser constraints (secure context + RP ID must be a domain, never an IP). Usable-today: `localhost` is exempt (build the `Fido2NetLib` flow on loopback first); for a lab box reached from a phone, a real wildcard cert via DNS-01 (Cloudflare + LE) dropped into the *existing* `management.https` bring-your-own-cert slot — no new node code — with the name resolved to the LAN IP. Distribution candidate: `B + mDNS` (vendor domain + call-home ACME wildcard, key on-box; mDNS discovery; stable per-install subdomain as RP ID so passkeys survive IP changes); DNS-rebinding named as the residual risk. Informs auth part 3 (passkeys)](#17-amendment-log)
