...
**Latest amendment:** [§17 entry 2026-06-03 — v2.2 arc V4: SREJ(mod-128) + segmentation integration](#17-amendment-log)
...
| 5.Z V2 ⬜ | SABME (mod-128) connect vs **direwolf** over net-sim AFSK1200 | as above vs direwolf (figure-faithful oracle; its source documents the SABME handshake). **Needs direwolf + an AGW accept/echo app in the stack — #177** (KISS alone bypasses direwolf's connected-mode engine; the engine is AGW-driven — see the #177 comment). Verify first that direwolf answers an inbound SABME at mod-128. |
...
| 5.Z V4 🚫 | mod-128 **windowed data transfer** (k>7, e.g. 16/32) vs **LinBPQ** | **BLOCKED vs LinBPQ** — no mod-128 link to BPQ (see the SABME row). Achievable vs **direwolf (#177)**. The mod-128 windowed-transfer path itself is proven in-process (`Mod128*ConformanceTests`, V5a ledger). |
| 5.Z V4 🚫 | mod-128 **loss recovery** (REJ + SREJ) under 30% net-sim loss vs **LinBPQ** | **BLOCKED vs LinBPQ** — same. Achievable vs **direwolf (#177)**. Proven in-process (`Mod128LossRecoveryConformanceTests`). |
| 5.Z V4 🚫 | **Segmentation/reassembly** (> N1 payload) vs **LinBPQ** | **BLOCKED vs LinBPQ** for the mod-128 case (segmentation is gated on a v2.2/negotiated link BPQ won't form). Achievable vs **direwolf (#177)**. Proven in-process (`SegmentationIntegrationConformanceTests`). NB: a *mod-8* segmentation interop is also possible only if BPQ negotiates the segmenter bit via mod-8 XID — untested; BPQ's XID clamp makes it unlikely. |
| 5.Z V3 ⬜ | **XID negotiation** vs **LinBPQ** | BPQ supports XID but **clamps to mod-8 / 256-byte** (`ProcessXIDCommand`: *"only accept Mod 8 and 256 Byte frames"*), so it cannot negotiate modulo-128 *up* via XID — only a mod-8 parameter exchange is possible. A mod-8 XID round-trip vs BPQ is a candidate follow-up row; modulo-128-via-XID needs direwolf (#177). Not implemented this leg. |
