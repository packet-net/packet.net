...
**Deferred:** INP3 (the triggered/RTT routing overlay — a self-contained later increment) and **real-BPQ-L4 interop** (validate `connect`/circuits against LinBPQ/XRouter over the docker stack — a documented follow-up; the concurrent agent owns the stack, and an in-process two-node harness covers it meanwhile).
...
| 9 ⬜ | **NET/ROM L3 origination + L4 circuits** vs **LinBPQ / XRouter** over net-sim | DEFERRED follow-up (documented). The full vanilla L3+L4 stack (NODES origination, the L4 virtual-circuit transport, `connect <alias>` routing) is proven in-process by a two-node harness (`NetRomL3L4IntegrationTests` over the in-memory radio bus + the deterministic `CircuitPairHarness`/`NodesExchangeTests` on a `FakeTimeProvider`); validating it against a real LinBPQ/XRouter (our NODES appears in BPQ's `NODES`; `connect`-through to a third node; sustained Info + choke + clean disconnect) waits until the interop docker stack is free (a concurrent agent owns it). |
