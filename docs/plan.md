...
- ✅ LinBPQ container interop: a real LinBPQ 6.0.25.23 image accepts our **FCS-bearing** AXUDP frames over the wire. (Correction, §17 2026-06-04: BPQ's BPQAXIP/UDP driver REQUIRES the FCS — it drops FCS-less datagrams as "Invalid CRC", source-verified — so this needs `includeFcs: true`; the pre-#299 claim that FCS-less "matches BPQAXIP" was wrong.)
...
| 4 | connected-mode over **AXUDP**: pdn → **LinBPQ** (BPQAXIP UDP) | ✅ `LinbpqViaAxudpConnectedMode.Pdn_Connects_Out_To_Linbpq_Over_Axudp_And_Exchanges_IFrames` — the pdn node host (`PortSupervisor` + `TransportFactory` → `AxudpKissModem`, `IncludeFcs = true`) dials BPQ's NODECALL through its own outbound connector; SABM/UA crosses the UDP tunnel, an `I`-frame round-trip (`P`-ports command → BPQ's ports list) flows, BPQ's RR S-frame acks are accepted (an unstripped FCS tail would break them), DISC/UA tears down. **FCS is mandatory on BPQAXIP/UDP** (source-verified `bpqaxip.c`; see §17 2026-06-04). |
...
| 4 | **AXUDP FCS regression guard** vs **LinBPQ** | ✅ `LinbpqViaAxudpConnectedMode.FcsLess_Sabm_IsDropped_FcsBearing_Sabm_GetsUa` — on the wire: an FCS-less SABM is silently dropped by BPQAXIP/UDP ("Invalid CRC") → no reply; the same SABM with the 2-octet CRC-16/X.25 FCS → BPQ answers UA (and BPQ's UA carries a valid FCS too). Locks in the `IncludeFcs = true`-for-BPQ requirement (§17 2026-06-04). |
