...
- **Apply:** the helper fetches the release `SHA256SUMS` **itself** (from a URL it derives from the repo + `node-v<target>`, never from the request) → downloads the matching per-arch `packetnet_<ver>_<arch>.deb` → **verifies its sha256** against that authoritative list → `dpkg -i` it (the detached `packetnet-update.service` oneshot survives the postinst restart) → health-gates on the node's own loopback `/healthz`, with `systemctl is-active` as the second opinion → on failure **rolls back** with `dpkg -i` of the retained prior `.deb` (`/var/cache/apt/archives` or a retained copy).
...
- **Targeted, dpkg-owned, never a dist-upgrade:** only the `packetnet` package moves and dpkg stays the sole owner of the files - no in-place self-mutation. The request file passes the target version + arch + download URL + expected sha256 + the node's effective health URL so the helper validates rather than trusting the caller.
- **The request file is attacker-writable, so every field is re-derived or re-validated.** The spool lives under the node's `RuntimeDirectory`, which the unprivileged `packetnet` user owns, and the helper runs as root - so the helper parses the download URL's **host** (it must be `github.com` / `*.github.com` / `*.githubusercontent.com`; globbing the whole URL let the allow-listed text sit in the path, packet.net#699 / C075), charset-gates the target version before it goes anywhere near a URL, refuses any health URL that is not a loopback `/healthz`, and treats the request's sha256 as a cross-check against the `SHA256SUMS` it fetched rather than as the expected value. `scripts/github-update-url-test.sh` pins the URL guards (the helper's `--check-url` / `--check-health-url` validate-only modes).
...
One oneshot serves both channels. Its `ExecStart` is the `packetnet-update` **dispatcher**, which reads the runtime-resolved channel the node spools to `/run/packetnet/update.channel` and execs the matching helper (`packetnet-apt-update` / `packetnet-github-update`). With no resolved channel it refuses rather than guessing. The target version + arch + download URL + expected sha256 + the node's effective loopback health URL are passed to the helper as a request file (`/run/packetnet/github-update.json`) so the helper validates rather than trusting the caller blindly.
...
**Deferred:** deepening the apt-channel health gate from `systemctl is-active` to a real `/healthz` probe (the github channel now probes `/healthz` first and falls back to `is-active`); **cosign/minisign** signing/verify (the checksum-only seam hardens to it later).
