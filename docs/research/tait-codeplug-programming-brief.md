...
**Reusable code.** `src/M0LTE.Tait.Ccdi/` in the sibling repo
[`M0LTE/M0LTE.Tait.Ccdi`](https://github.com/M0LTE/M0LTE.Tait.Ccdi) (it was `src/Packet.Radio.Tait/`
here when this was written, then `src/M0LTE.Radio.Tait/` in the sibling repo's first shape)
already has the serial plumbing you may want: an
