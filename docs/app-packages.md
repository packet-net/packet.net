...
session:                     # OPTIONAL — per-connection attachment (the verb is packet.command above)
...
- **Command** is the only one in the manifest because it *is* the app's identity (a generic "bbs" app legitimately calls itself `BBS`). It registers a bare node-prompt verb — typing `BBS` at the node connects you to the app (for a `service` app, a loopback connect to its bound callsign; for a `session` app, it triggers the per-connection attachment). It **replaces** the old `session.match` — there is **no back-compat parsing** (pre-1.0, single deployment; the bundled app manifests move to `packet.command` in the same change, same discipline as the forwarding-config rename). Note `packet.command` (the verb word) is distinct from `session.command` (the executable for `kind: process`). Optional: omit it and the app is reachable only by callsign/alias.
...
- **The node owns the callsign.** It resolves each enabled packet app's callsign (the `apps[].callsign` pin, else `<node-base>-N` probing for a free SSID) and hands it to the app via **`PDN_APP_CALLSIGN`**. Apps bind *that* — they no longer derive their own identity from `PDN_NODE_CALLSIGN`. (Migration: every app that self-derives `<node>-N` today moves to `apps[].callsign` + `PDN_APP_CALLSIGN` — **pdn-bbs** (`bbs.yaml callsign:`), **DAPPS** (`DAPPS_CALLSIGN` env), **pdn-bpqchat** (its `SSID`/derive, default `-4`), and **pdn-convers** (same). The node's free-SSID probe-walk replaces each app's own.)
...
- `GET /api/v1/apps/packages` - the admin inventory: every discovered package + every inline entry + every **configured-but-not-installed** app (an `apps:` override naming a package no root holds; `installed: false`, no manifest data, [#738](https://github.com/packet-net/packet.net/issues/738) item 2; the supervisor also logs a Warning per *enabled* one at bring-up and on every reconcile, so a lost payload stops looking like a healthy node), with `{id, name, version, description, icon, capabilities, enabled, source: package|inline, installed, service: none|managed|external, state: Stopped|Starting|Running|Backoff|Faulted|External, pid?, detail?, forwards: [{listen, target, tls}]}`, plus the packet identity: `command` (the effective verb), `callsign` (the node-RESOLVED callsign, a pin or an auto-assigned `<node-base>-N`), `pinnedCallsign` (the owner's literal pin, null when the callsign was auto-assigned) and `netromAlias`/`netromQuality`. Read scope to view; **admin scope to mutate**.
...
- `PUT /api/v1/apps/packages/{id}/identity` - the packet-identity overrides `{command, callsign, netromAlias, netromQuality}` (admin). A **full replace**: an absent or blank field CLEARS that override, so a caller keeps a value by sending it back. That is why the inventory projects `pinnedCallsign` separately: an editor seeded from the resolved `callsign` would pin an auto-assignment, and one that sends `null` (as the panel's identity editor did) silently drops an existing pin on a verb- or alias-only edit ([#702](https://github.com/packet-net/packet.net/issues/702) C043).
