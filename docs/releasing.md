...
The generator walks `git log` from the previous tag **in the same series** (`node-v*` or `headend-v*`, version-sorted, so `0.13.9` -> `0.13.10` orders correctly) to the tag being released, and buckets each subject by its conventional-commit type into **Breaking / New / Fixed / Performance / Changed / Documentation / Internal**. A subject whose prefix is not a known type (this repo also writes bare subsystem prefixes such as `catalog:` or `tune:`) keeps the prefix as a bold scope and lands in **Changed** rather than being demoted to internals - those are usually the user-visible commits. It ends with a compare link and a commit count, then a single line pointing at the install docs, taken from `.github/release-notes/<series>.md`.
