> ⚠️ **SUPERSEDED (2026-06-14)** - The DNS-01 path (a real cert for `pdn.<domain>` via acme.sh + Cloudflare DNS-01) and the `B + mDNS` distribution pattern described below are **retired**. The node-access decision is now **HTTP-first** (TLS is the edge's job) with **embedded Tailscale** (a `tsnet` Go sidecar) as the blessed remote + passkey path: it provides a real, browser-trusted cert for `pdn.<tailnet>.ts.net` with no public DNS, port-forward, ACME, or DNS-01 plumbing. See [`network-access.md`](network-access.md) for the decision and slice plan. This document is **kept as preserved analysis** of the secure-context / RP-ID constraints (which still hold) and the alternatives considered - it is no longer the plan.
