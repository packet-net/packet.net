...
- **SDL state machine is a NuGet dependency** (`Packet.Ax25.Sdl`, built by `m0lte/ax25sdl`). Do **not** edit/regenerate it from this repo. If rung 1 shows an engine-emission bug (e.g. #79), raise it against `m0lte/ax25sdl`, publish, bump the pin in `Directory.Packages.props`. The bench *drives* the engine; it does not modify the SDL.
