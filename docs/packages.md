...
Radio control (`IRadioControl`: RSSI, hardware carrier-sense/DCD, PTT) and station-rig CAT control (`IRigControl`: frequency, mode, PTT, SWR/power meters) used to ship from here as `Packet.Radio*` and `Packet.Rig*`. Neither is packet radio, and the `Packet.` prefix implied a coupling to the AX.25 stack that most of that code never had, so both now live in sibling repos of their own and arrive here as ordinary NuGet dependencies: [`M0LTE/M0LTE.Rig`](https://github.com/M0LTE/M0LTE.Rig) publishes [`M0LTE.Rig`](https://www.nuget.org/packages/M0LTE.Rig) (both `IRadioControl` and `IRigControl` since 2026-09-06 - the two names did not carry the distinction on their own), [`M0LTE.Rig.Hamlib`](https://www.nuget.org/packages/M0LTE.Rig.Hamlib) and [`M0LTE.Rig.Flrig`](https://www.nuget.org/packages/M0LTE.Rig.Flrig), and [`M0LTE/M0LTE.Tait.Ccdi`](https://github.com/M0LTE/M0LTE.Tait.Ccdi) publishes [`M0LTE.Tait.Ccdi`](https://www.nuget.org/packages/M0LTE.Tait.Ccdi), the Tait TM8100/TM8200 CCDI driver (that repo was `M0LTE/M0LTE.Radio` until the 2026-09-06 rename). Type names and behaviour are unchanged; the namespaces follow the package ids.
