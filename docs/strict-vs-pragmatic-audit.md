...
| What | Old (buggy) | Correct (now default) | Evidence |
|---|---|---|---|
| HDLC Optional Functions PV octet order | LSB-octet first (`field & 0xFF` first) - e.g. mod-8+SREJ+SREJ-multi serialised `84 A4 22` | **MSB-octet first** (`field >> 16` first) - the same selection serialises `22 A4 84` | **AX.25 v2.2 §3.8** (normative): multiple-octet fields are sent "high-order octet first". In our bit-numbering the always-1 Extended-Address bit is bit 7 (the low-order octet); §3.8 puts that low octet *last* on the wire, so the §3.8-correct PV for the figure's selection is `22 A8 82`. Figure 4.6 instead prints `82 A8 22` - the ext-addr/low octet *first* - i.e. LSB-octet first, contradicting §3.8 (a figure-rendering error). **Dire Wolf** `xid.c` and **LinBPQ** `L2Code.c` both serialise high-order octet first on the wire (direwolf writes `(x>>16),(x>>8),x` and parses `pval=(pval<<8)+*p`; LinBPQ writes `xidval>>16` first and parses `value=(value<<8)+*p++`), i.e. §3.8-conformant MSB-octet-first. **Proven on the wire** (`SrejXidViaNetsim`, `Category=Interop`): BPQ accepts the MSB-first PV and answers XID selecting SREJ-multi + mod-8; it silently drops the LSB-first PV (fails BPQ's `OPMustHave`/`OPSREJMult` check → no SREJ ever negotiated). |

`ToOctets`/`FromOctets` carry a `lsbOctetFirst` parameter (default `false`) so the legacy layout can be reproduced for regression study, but the production path (`XidInfoField.Encode`/`TryParse`, the management-data-link offer) always uses the spec-correct MSB-first order. **Cross-stack note for the lead: ax25-ts must apply the identical octet-order fix** - until it does, the two stacks emit byte-reversed HDLC Optional Functions and only one will negotiate SREJ with BPQ/direwolf. This is a behaviour change, not a named-flag-inventory change, so `parity-check.mjs` will *not* flag it; the TS leg needs an explicit matching change. **Upstream**: candidate `packethacking/ax25spec` issue - Figure 4.6 prints the HDLC Optional Functions PV least-significant-octet first (`82 A8 22`), contradicting the normative §3.8 "high-order octet first" (which §3.8 itself muddies with a "(little endian)" aside; implementations follow the "high-order octet first" phrase). A second Fig 4.6 rendering defect alongside the ABM off-by-one below; flagged for Tom.

### XID pre-SABM SREJ negotiation - the LinBPQ accommodation (`PreConnectXidNegotiatesSrej`)

Genuine de-facto pragmatism (a *connect-strategy* knob, not a figure defect and not a parser leniency), so it lives on `Ax25ListenerOptions` (mirrored in the live-reseed `Ax25SessionParameters`), default **on**.
...
| Location | Choice | Why it's not a flag |
|---|---|---|
| `ClassesOfProcedures.ToOctets` puts Balanced-ABM at **bit 0** (half-duplex ⇒ `0x21 0x00`) | Figure 4.5's table and §6.3.2 ¶1077 ("Bit 0 is always a 1") put ABM at bit 0. Figure 4.6's worked example instead shows `0x22 0x00` - it placed the always-1 ABM bit at position 1. (The HDLC-Optional-Functions PV is serialised most-significant-octet-first per §3.8 - see the octet-order section above - the order direwolf / BPQ use on the wire; Fig 4.6 prints *that* field LSB-first, a separate figure error.) We follow the normative table/prose for the ABM bit, so it is bit 0 and half-duplex encodes `0x21`. | Spec-table-vs-figure contradiction, like the APRS `h`-timestamp row - we follow the table. The duplex selection (bit 5) - the only field a peer reads - is identical either way (it lands in the same octet under either octet order), so there is no interop consequence. **Upstream**: candidate `packethacking/ax25spec` issue (Fig 4.6 ABM byte should be `0x21`, not `0x22`); flagged for Tom, not filed here. |
...
- Each flag has a paired case in the conformance vectors (most in [`deviations.json`](https://github.com/packet-net/aprs-vectors/blob/main/cases/deviations.json)): `Lenient` accepts with a warning naming the defect, `Strict` gives the case's strict result (usually a rejection), and `tests/Packet.Aprs.Tests/Vectors` checks that turning off only that flag gives the strict result too. A test fails the build if a flag has no case, and another that `Strict` / `Lenient` are all-off / all-on.
...
- Raw NMEA sentences whose `*hh` checksum doesn't match: the sentence is corrupt (NMEA 0183), and Ham::APRS::FAP rejects it too. See [`interpretations.md`](https://github.com/packet-net/aprs-vectors/blob/main/interpretations.md#raw-nmea-with-a-checksum-that-doesnt-match).
...
## Node surface: per-port `link:` dial policy (#724, 2026-08-17) - not a library flag

A peer that **ignores** a SABME rather than rejecting it is the one v2.2 interop case the engine deliberately cannot handle: FRMR and DM degrade to v2.0 inside the SDL (`Ax25Spec45` / `Ax25Spec48`), but AX.25 v2.2 section 6.3.1 mandates no silent fallback, so a dial that draws no answer at all can only exhaust its budget. That is not a parse leniency and it is not a session quirk; it is a *dial policy*, and the node is where it belongs.

`PortConfig.link` (`Packet.Node.Core.Configuration.PortLinkConfig`) makes it a named per-port setting: `dial: auto | v22 | v20` and `preConnectXid: auto | on | off`, absent = all-auto = the node's historical behaviour. `v20` is the declaration for a BPQ / LinBPQ or older-TNC-facing port. Under `auto`, `Ax25OutboundConnector` degrades once, and only on the unambiguous evidence: the dialled peer sent back not one frame during the whole attempt (watched on the port's frame trace - the exception type alone cannot separate this from a DM refusal, because the SDL's give-up at `RC == N2` surfaces as the same teardown). It then records the negative in `PeerCapabilityCache` and retries mod-8 on the same cached session, so the first connect still succeeds and later ones lead with a SABM.

**No library flag and no widened listener surface**, so the ax25-ts parity check is untouched: the policy is projected onto the two existing, already-parity-tracked `Ax25ListenerOptions` members (`PreferExtendedConnect`, `PreConnectXidNegotiatesSrej`), which the node previously never set from config. An engine-side quick silent-fallback quirk remains a separate future item, and would need its TS leg.

## Listener routing at the session modulo (#696, 2026-08-16) - not a leniency

The inbound pump must parse a frame before it can route it, and it cannot know the session modulo until it has routed, so its routing parse is **modulo-8**. That reads an extended (mod-128) I frame acceptably - the second control octet lands on the PID - but an extended **supervisory** frame's second control octet looks like an information field on an S frame, which §3.5 does not permit. `Ax25Frame.TryParse` therefore rejected the whole frame whenever `AllowInfoOnSupervisoryFrames` was off, i.e. under `Strict` (and `Xrouter`, which is a copy of `Strict`). SABME and UA are U frames, one octet in both modes, so a v2.2 link came **up** on such a port and then had every RR / RNR / REJ / SREJ dropped before trace and dispatch. `Lenient` was unaffected: it captured the second octet as info and `ReparseAtSessionModulo` fixed the reading once the session was matched.

The fix is in the routing, not in the options: on a failed modulo-8 parse the pump retries at modulo-128 and accepts that reading **only when the address pair already has a cached session with `IsExtended` set**. So it is not a widening of what a strict port accepts from the world - the frames it now admits are exactly those whose second control octet genuinely is a control octet, on a link this station itself negotiated to mod-128. A mod-8 link's frames are untouched (they parse at modulo-8 or not at all), and a strict port stays deaf to a malformed S frame from any peer it has no extended link with. **No flag, no preset change, no ax25-ts parity surface** - the paired tests are `Ax25ListenerExtendedSupervisoryRoutingTests` (Strict + SABME + extended RR delivered; Strict with no extended session still deaf; the mod-8 link unchanged; Lenient unchanged).
...
|---|---|---|---|---|---|
| ~~`Ax25Spec38SrejSelectiveRetransmit`~~ | *(retired 2026-08-03)* | figc4.5 SREJ-received drew a generic "Push frame onto queue" + go-back-N "Invoke Retransmission"; the quirk rewrote that into the figc4.4 single-frame selective retransmit. **`ax25sdl` 0.10.2 ships the corrected figc4.5** (ax25sdl#78 ← ax25spec#65), which does single-frame selective natively on all four SREJ paths - so the workaround became inert (neither verb it acted on survives in the tables) and was removed with its synthetic tests. The behaviour it protected is unchanged and now covered end-to-end by `DataLinkSrejUnderLossTests` against the real tables. Its "remove when" condition (packet.net#227) is met. | - | - | done |
| ~~`Ax25Spec40DiscardOutOfWindowIFrames`~~ | *(retired 2026-09-10)* | figc4.4/figc4.5's out-of-sequence `I_received` arm had no receive-window guard, so a duplicate of an already-acknowledged frame was SREJ'd or REJ'd; with an empty sender queue that SREJ names a frame that will never exist, nothing clears it and §4.4.4 re-sends it on every poll (observed on air against BPQ 6.0.24.79, 2026-09-09). The quirk OR'd the out-of-window test into the figure's own `reject_exception` decision. **`ax25sdl` 0.11.0 ships the corrected figures** (ax25sdl#81 <- ax25spec#40), which draw the test natively as a `V(r) < N(s) < V(r) + k?` decision ahead of the reject-exception/SREJ split, so the workaround was removed and the atom bound instead. Behaviour is unchanged: the same conformance and adversarial properties pass across the retirement, which is the proof the quirk was faithful. | - | - | done |
| `Ax25Spec48DmRejectionDegradesToV20` | figc4.6 (AwaitingV22Connection) DM-received: F=0 (`t11_dm_received_no`) drops passively to AwaitingConnection; **F=1 (`t11_dm_received_yes`) tears down to Disconnected** as a §975 refusal, **no fallback** - so a v2.2-preferred connect against a peer that DMs our polled SABME (P=1, answered F=1) dies with `IsExtended` stuck true. | A DM in AwaitingV22Connection (either F-branch) means "peer can't do v2.2": force version 2.0 and re-establish via SABM (run figc4.6's own `t14_frmr_received` v2.0-fallback action chain), the DM analogue of the FRMR fallback (`Ax25Spec45`). Scoped to the extended-connect state - a DM to a subsequent SABM in AwaitingConnection stays a genuine refusal → Disconnected. | A DM is the unambiguous "no v2.2" signal (the peer doesn't recognise SABME); a peer that DMs our *polled* SABME (P=1) answers F=1 (so XRouter hits the teardown branch). **Verified on the wire**: XRouter answers our SABME(P=1) with DM(F=1) → figc4.6 routes to Disconnected (packet.net interop `XrouterViaNetsimExtendedMode`). De-facto: direwolf's author degrades rather than honour the figure-literal figc4.6 teardown. **Upstream: [packethacking/ax25spec#48](https://github.com/packethacking/ax25spec/issues/48)** (filed). | `true` | ax25sdl ships a corrected figc4.6 (DM-degrades-to-v2.0) |

(The figc4.4/4.6/4.7 figure-defect quirks `Ax25Spec41/42/43/44/45/46/47` follow the identical pattern and live on the same `Ax25SessionQuirks` record - see the XML docs there and the §17 amendment log; not all are tabulated above.)
