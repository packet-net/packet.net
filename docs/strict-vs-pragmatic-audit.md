...
## AXUDP / AXIP-over-IP FCS framing (`AxudpTransport.IncludeFcs` — transport-config, not parser-leniency)

Strictly this is a **transport-config** matter, not an `Ax25ParseOptions`/`AprsParseOptions` parser-leniency one (the AX.25 parser never sees the FCS — `AxudpKissModem` strips it before the listener parses), so it lives in the AXUDP code + interop tests, not in a `…ParseOptions` flag. It is captured here because the **strict-vs-pragmatic + de-facto-survey discipline is exactly the same** (§2: "any implementation is an interop target, not reference truth; survey the real ones"), and the surrounding docstrings point here for the citation trail.

**The question (#299→#301→this PR):** PR #299 added the AXUDP transport (`AxudpKissModem` over `AxudpSocket`) defaulting **FCS-off** "for a pdn↔pdn tunnel", on a (wrong) belief that FCS-less "matches BPQAXIP". #301 source-verified that BPQAXIP/UDP actually *requires* the FCS and fixed the docs, but left the default FCS-off. The open question: does **any** real third-party AXIP/AXUDP implementation send/accept FCS-**less** frames, or did pdn invent a self-only wire format?

**Verdict: FCS-less is a pdn-invented, self-only wire format. Every real AXIP/AXUDP implementation surveyed mandates the 2-octet AX.25 FCS; none omits it or offers an FCS-less mode.** So the interoperable default is **FCS-on**, and that is now the default across `AxudpSocket.SendAsync`, `AxudpKissModem`, and `AxudpTransport.IncludeFcs` (all flipped `false`→`true` in this PR). All surveyed peers compute the **identical** CRC (poly 0x1021, init 0xffff, final ^0xffff, **low byte first** on the wire, good residue `0xf0b8`) — byte-for-byte our `Packet.Core.Crc16Ccitt` — so the FCSes interoperate exactly.
...
| **pdn (`Packet.Axudp`/`AxudpKissModem`)** | **Included by default (this PR);** FCS-less retained as a non-standard opt-out. | **Yes** — `IncludeFcs` (now default `true`). | **FCS-on (was FCS-off pre-this-PR)** | `AxudpSocket.SendAsync(includeFcs: true)`, `AxudpKissModem(includeFcs: true)`, `AxudpTransport.IncludeFcs = true`. |

### Flag

| Flag | Spec / de-facto | FCS-less reading | Driver for the default | Default | Notes |
|---|---|---|---|---|---|
| `IncludeFcs` (on `AxudpTransport`; ctor param on `AxudpKissModem` / `AxudpSocket.SendAsync`) | RFC 1226 + rfc1226-bis + ax25ipd + BPQAXIP + XRouter + JNOS: the 2-octet AX.25 FCS (CRC-16-CCITT, low byte first) is part of the encapsulated payload and is **mandatory**. | `false` = body alone, **no FCS**. **No surveyed real implementation sends or accepts this** — it is a pdn-only minimal form. | **The dominant AXUDP peer (LinBPQ BPQAXIP) — and every other real peer — requires the FCS.** An out-of-the-box AXUDP port must talk to them. | **`true`** (interoperable). | Kept `false` as a *named, documented* opt-out for a symmetric **pdn↔pdn** tunnel only (the one place FCS-less is real — both ends pdn, both opting out). Receiver strips+validates the FCS when on (mandatory: `Ax25Frame.TryParse` rejects an S-frame with a trailing FCS tail, so an unstripped tail would silently drop every RR/RNR ack). `false`↔`true` is a **symmetric link parameter** — both ends must agree. Verified: out-of-the-box default = FCS-on (`AxudpKissModemTests.Default_includeFcs_is_true…`); pdn↔pdn FCS-on round-trip (`AxudpNodeToNodeIntegrationTests`); BPQ on-the-wire FCS guard (`LinbpqViaAxudpConnectedMode.FcsLess_Sabm_IsDropped_FcsBearing_Sabm_GetsUa`, `Category=Interop`). |
