...
| [`M0LTE/tait-codeplug`](https://github.com/M0LTE/tait-codeplug) | AGPL-3.0 Tait TM8100/TM8200 codeplug programmer: the `tait-codeplug` CLI + the `M0LTE.Tait.Codeplug` NuGet package. Provisions a radio for the `M0LTE.Tait.Ccdi` driver. Split out of this repo on 2026-08-20. |
| [`M0LTE/M0LTE.Rig`](https://github.com/M0LTE/M0LTE.Rig) | AGPL-3.0 rig and radio control: `M0LTE.Rig` (the `IRigControl` CAT abstraction plus the `IRadioControl` / `IRadioSideChannel` radio-control abstraction, folded in on 2026-09-06) + the `M0LTE.Rig.Hamlib` and `M0LTE.Rig.Flrig` backends. Split out of this repo on 2026-09-05. |
| [`M0LTE/M0LTE.Tait.Ccdi`](https://github.com/M0LTE/M0LTE.Tait.Ccdi) | AGPL-3.0 Tait TM8100/TM8200 CCDI driver: `M0LTE.Tait.Ccdi`. Split out of this repo on 2026-09-05 as `M0LTE/M0LTE.Radio` (`M0LTE.Radio` + `M0LTE.Radio.Tait`); renamed and reshaped on 2026-09-06 when the `IRadioControl` / `IRadioSideChannel` abstraction moved into `M0LTE.Rig`. |

All of the above are public (the three `M0LTE/*` repos sit outside the org, under Tom's own account, because none of them is about packet radio). The [`packet-net`](https://github.com/orgs/packet-net/repositories) organisation holds more besides - the soundmodem engine, apps that run on the node, the net simulator - but the table above is the set this repository is directly coupled to.

The two `M0LTE.Rig` / `M0LTE.Tait.Ccdi` repos are the only ones this repo depends on that it does not own: `Packet.Ax25.Radio` and `Packet.Ax25.Radio.Tait` here are thin AX.25 adapters over them, and `Packet.Node.Core` consumes them directly. The dependency runs one way only, so nothing there is coupled back to the packet stack. Release ordering is in [`releasing.md`](releasing.md) step 0a.
