...
| [`M0LTE/tait-codeplug`](https://github.com/M0LTE/tait-codeplug) | AGPL-3.0 Tait TM8100/TM8200 codeplug programmer: the `tait-codeplug` CLI + the `M0LTE.Tait.Codeplug` NuGet package. Provisions a radio for the `M0LTE.Radio.Tait` driver. Split out of this repo on 2026-08-20. |
| [`M0LTE/M0LTE.Radio`](https://github.com/M0LTE/M0LTE.Radio) | AGPL-3.0 radio control: `M0LTE.Radio` (the `IRadioControl` / `IRadioSideChannel` abstraction) + `M0LTE.Radio.Tait` (the TM8100/TM8200 CCDI driver). Split out of this repo on 2026-09-05. |
| [`M0LTE/M0LTE.Rig`](https://github.com/M0LTE/M0LTE.Rig) | AGPL-3.0 CAT rig control: `M0LTE.Rig` (the `IRigControl` abstraction) + the `M0LTE.Rig.Hamlib` and `M0LTE.Rig.Flrig` backends. Split out of this repo on 2026-09-05. |
...
The two `M0LTE.Radio` / `M0LTE.Rig` repos are the only ones this repo depends on that it does not own: `Packet.Ax25.Radio` and `Packet.Ax25.Radio.Tait` here are thin AX.25 adapters over them, and `Packet.Node.Core` consumes them directly. The dependency runs one way only, so nothing there is coupled back to the packet stack. Release ordering is in [`releasing.md`](releasing.md) step 0a.
