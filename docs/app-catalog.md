...
## Catalog maintenance - `scripts/regen-catalog.sh` (O3)

Bumping an app no longer means hand-editing the version + copying six sha256s. `scripts/regen-catalog.sh` regenerates `catalog/apps.yaml` from each app's **latest GitHub release**:

```sh
scripts/regen-catalog.sh            # rewrite catalog/apps.yaml in place to the latest releases
scripts/regen-catalog.sh --app bbs  # only this app (repeatable)
scripts/regen-catalog.sh --check    # drift guard: nonzero exit if any entry lags its latest release; writes nothing
scripts/regen-catalog.sh --print    # print the version+sha block per app (paste-friendly); writes nothing
```

For every catalog entry it resolves the app's GitHub repo (from the entry's `homepage:`, falling back to a built-in id→repo map), runs `gh release view --json tagName,assets` for the **latest** release, maps each catalog RID to the matching release asset (assets-kind binaries by the `<tool>-<rid>` filename; deb-kind by the Debian arch token, `linux-x64→amd64` etc.; pdnapp by `.pdnapp`), and rewrites the entry's `version:`, every artifact `url:`, and every `sha256:` **in place**. It **preserves** the file's comments, ordering, and the `name`/`description`/`icon`/`capabilities`/`homepage` fields, and it covers all three artifact kinds.

The sha256 pin comes straight from **GitHub's per-asset `digest` (`sha256:…`)** - the byte hash GitHub records on upload, the same value `build-deb.sh` used to copy by hand - so no artifact bytes are downloaded and "vetted" still means *Tom ran regen against a chosen release*. The rewrite is idempotent (re-running with no upstream change leaves the file byte-identical), so it is safe to wire `--check` into CI or a pre-release step as the standing freshness/drift guard (this is also what keeps O4's manifest-drift from recurring). Requires `gh` (authenticated for the `packet-net` org) and `python3` with PyYAML.
...
- `scripts/regen-catalog.sh` (+ `scripts/regen-catalog.py`) - the O3 regen: rewrites the catalog's version/url/sha256 pins in place from each app's latest GitHub release (`--check` drift guard, `--print` paste mode). See § Catalog maintenance.
...
- **O2 - apt-as-store complementary CLI channel. ⏸ BLOCKED on the OARC apt repo + a design decision (#475, still open for this).** bpqchat/convers/bbs already ship as proper Debian packages, so `apt install pdn-bpqchat` (once the OARC apt repo carries them) is a real alternative install path: Debian deps/trust + a CLI install with root. It coexists with the catalog rather than replacing it - the catalog is the browser/unprivileged/air-gapped path (via `.deb` *extraction* under `/var/lib`), apt is the CLI/root path (via `.deb` *installation* under `/usr/share`). Same artifact, two mechanisms. **Why it can't be built now:** (1) it depends on the OARC apt repo, which is maintainer-owned and not yet stood up (the same dependency that dropped the apt self-update channel - see `node-self-update-design.md`); (2) it needs a design decision on how an apt-installed app integrates with catalog/discovery. Open design questions for Tom: **does an apt-installed app appear in "Available apps" as already-installed** (discovery already scans `/usr/share/packetnet/apps`, so a `.deb` that lands a manifest there is discovered - but it carries no `.pdn-install.json` marker, so uninstall correctly refuses it; is "manage via apt, not the panel" the right UX, or do we want a read-only "installed via apt" badge)? **Does the catalog advertise an apt name** alongside the URL pins so the UI can offer "or: `apt install …`"? **Do the two channels share the trust root** (apt's repo GPG vs the catalog's deb-integrity-anchored sha pins)? Until the repo exists and these are answered, this stays **named, not built** - the reason #475 is left open.
- **O3 - catalog maintenance / regen. ✅ RESOLVED (#475) - `scripts/regen-catalog.sh`.** A future bump is now `scripts/regen-catalog.sh`, not hand-editing six hashes. See § Catalog maintenance below for the script + how to run it. "Vetted = Tom ran regen against a chosen release" holds: the script pulls each asset's GitHub-recorded sha256 digest from the latest release and rewrites the entry in place.
- **O4 - manifest version drift. ✅ RESOLVED (#475) - manifests caught up.** The earlier drift (bpqchat manifest `0.0.1` vs release `v0.1.0`; convers `0.1.0` vs `v0.1.2`) is gone: as of 2026-06-18 every vetted app's in-repo `pdn-app.yaml` `version:` equals its latest release tag (dapps 0.34.2, bpqchat 0.2.1, convers 0.1.3, bbs 0.2.35), so the spurious-"update available" condition no longer occurs. The design stands as intended - the catalog `version` tracks the release tag (the real artifact); the installed package shows its manifest's version; they now agree. The `--check` mode of the regen script (above) is the standing drift guard: it flags any catalog entry whose pins lag the latest release, which is the only way O4 can recur. The deeper "compare on the install marker, not the manifest" option remains available if an app's manifest ever lags again, but is not needed today.
