...
(`M0LTE.Rig`) and `IRigControl` (`M0LTE.Rig`) - the **radio behind the
modem**, for stations whose radio has a serial control channel (RSSI, hardware
carrier-sense, CAT). They ship from a sibling repo of its own; they're
...
dotnet add package M0LTE.Rig               # the packet-medium (radio) and station-control (CAT) seams
...
dotnet add package M0LTE.Tait.Ccdi         # Tait CCDI driver, if your radio is a Tait
```
...
!!! note "Published packages"
    ...
    publication matrix. The radio and rig seams themselves come from
    [`M0LTE/M0LTE.Rig`](https://github.com/M0LTE/M0LTE.Rig) (both interfaces,
    plus the Hamlib/Flrig CAT backends), and the Tait CCDI driver from its own
    sibling repo, [`M0LTE/M0LTE.Tait.Ccdi`](https://github.com/M0LTE/M0LTE.Tait.Ccdi);
    they publish as `M0LTE.Rig*` / `M0LTE.Tait.Ccdi`.
