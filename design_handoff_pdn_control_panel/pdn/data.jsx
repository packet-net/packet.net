// ============================================================
// pdn — mock data, modelled on docs/node-ui-design.md §6
// Real record field names from Packet.Node.Core.Configuration,
// Packet.NetRom.Routing, and the new Slice-3 read models.
// ============================================================

// 6.1 NodeConfig tree (exists in codebase) ---------------------
const NODE_CONFIG = {
  schemaVersion: 3,
  ...
  ports: [
    {
      ...
      profile: "fast-il2p-1200",
      ax25: { t1Ms: 3000, t2Ms: 300, t3Ms: 180000, n2: 8, windowSize: 4, maxCachedPeers: 64 },
      kiss: { txDelay: 300, persistence: 63, slotTime: 100, txTail: 50 },
    },
    {
      id: "uhf-2",
      enabled: true,
      ...
      ax25: { t1Ms: 4000, t2Ms: 500, t3Ms: 180000, n2: 10, windowSize: 4, maxCachedPeers: 64 },
      kiss: { txDelay: 400, persistence: 63, slotTime: 100, txTail: 80 },
    },
    {
      id: "link-dn",
      enabled: true,
      transport: { kind: "axudp", host: "44.131.91.2", port: 10093, localPort: 10093 },
      profile: null,
      ax25: { t1Ms: 2000, t2Ms: 200, t3Ms: 180000, n2: 8, windowSize: 7, maxCachedPeers: 32 },
      kiss: null,
    },
    {
      id: "hf-300",
      enabled: false,
      transport: { kind: "serial-kiss", device: "/dev/ttyUSB1", baud: 38400 },
      profile: "robust-hf",
      ax25: { t1Ms: 8000, t2Ms: 1500, t3Ms: 300000, n2: 12, windowSize: 2, maxCachedPeers: 16 },
      kiss: { txDelay: 250, persistence: 32, slotTime: 100, txTail: 100 },
    },
  ],
...
// Field apply-impact map (which edits are hot vs disruptive) ---
// drives the per-field badges and the reconcile preview.
const APPLY_IMPACT = {
...
const NETROM = {
  ...
  destinations: [
    { destination: "GB7BNS", alias: "BNSGW", bestRoute: 0, routes: [
      { neighbour: "GB7BNS", quality: 203, obsolescence: 6, inp3: { targetTimeMs: 240, hopCount: 1 } },
    ]},
    { destination: "GB7CIP", alias: "CIPGW", bestRoute: 0, routes: [
      { neighbour: "GB7CIP", quality: 188, obsolescence: 6, inp3: { targetTimeMs: 410, hopCount: 1 } },
      { neighbour: "GB7BNS", quality: 142, obsolescence: 4, inp3: { targetTimeMs: 980, hopCount: 2 } },
    ]},
    { destination: "G1MNW-2", alias: "READNG", bestRoute: 0, routes: [
      { neighbour: "G8PZT-7", quality: 199, obsolescence: 6, inp3: { targetTimeMs: 180, hopCount: 1 } },
    ]},
    { destination: "GB7MAX", alias: "MAXGW", bestRoute: 0, routes: [
      { neighbour: "G8PZT-7", quality: 176, obsolescence: 5, inp3: { targetTimeMs: 620, hopCount: 3 } },
      { neighbour: "GB7CIP", quality: 151, obsolescence: 4, inp3: null },
    ]},
    { destination: "MB7UWS", alias: "UWSNOD", bestRoute: 0, routes: [
      { neighbour: "MB7UWS", quality: 168, obsolescence: 6, inp3: null },
    ]},
    { destination: "GB7SAN", alias: "SANGW", bestRoute: 0, routes: [
      { neighbour: "GB7BNS", quality: 133, obsolescence: 3, inp3: { targetTimeMs: 1340, hopCount: 4 } },
    ]},
  ],
};

// 6.4 read models: node status --------------------------------
const NODE_STATUS = {
  callsign: "GB7RDG", alias: "RDGGW", grid: "IO91nl",
  version: "0.7.0-rc2 (b57f327)",
  uptimeSeconds: 1987260, // ~23 days
  portsUp: 3, portsTotal: 4,
  sessionCount: 4,
  netrom: { neighbours: 4, destinations: 6, inp3Enabled: true },
};

// 6.4 port status (live state) --------------------------------
const PORT_STATUS = {
...
// 6.3 monitor frame seed pool ---------------------------------
const FRAME_TYPES = ["UI","SABM","SABME","I","RR","RNR","REJ","SREJ","FRMR","UA","DISC","DM","XID"];
const CALLS = ["M0LTE","2E0XYZ","G4APL-1","G8PZT-7","GB7BNS","GB7CIP","MB7UWS","G1MNW-2","M7ABC","GB7RDG","2E1FOX","G0HWC"];
const PORTS_LIST = ["vhf-1","uhf-2","link-dn"];
const PIDS = { "0xF0": "No layer 3", "0xCF": "NET/ROM", "0xCC": "ARPA IP", "0x08": "Segmentation" };

function randItem(a){ return a[Math.floor(Math.random()*a.length)]; }

// Build one synthetic MonitorEvent with a full decode payload.
let _frameSeq = 9000;
function makeFrame(now){
  const type = randItem(FRAME_TYPES);
  const dir = Math.random() > 0.5 ? "in" : "out";
  const port = randItem(PORTS_LIST);
  let source = randItem(CALLS), dest = randItem(CALLS);
  ...
  const length = isI ? 20 + Math.floor(Math.random()*236)
              : type === "UI" ? 12 + Math.floor(Math.random()*60)
              : 15;
  let summary;
  ...
  // synthetic raw octets for the hex view
  const raw = [];
  ...
  return {
    ...
    pid: pidKey,
    pidName: pidKey ? PIDS[pidKey] : null,
    ns, nr, pf,
    command: dir === "out",
    length,
    summary,
...
// Seed a backlog so the monitor isn't empty on load.
...
// 6.4 link stats (per-link rollup) ----------------------------
const LINK_STATS = [
...
function hex(n, w){ return n.toString(16).toUpperCase().padStart(w||2,'0'); }

// ============================================================
// Operator-facing config model (Phase-5 redesign)
// ============================================================

// display label for transport kinds (ninotnc is special-cased on purpose)
const KIND_LABEL = { "kiss-tcp": "kiss-tcp", "serial-kiss": "serial-kiss", "nino-tnc": "ninotnc", "axudp": "axudp" };
// which transports speak KISS (so we only show modem params where they apply)
const KIND_USES_KISS = { "kiss-tcp": true, "serial-kiss": true, "nino-tnc": true, "axudp": false };

// NinoTNC modem modes — served by the node (it knows its firmware's table).
// On the wire a NinoTNC is always 57600 baud USB-serial; "mode" sets the
// radio-side modulation/baud. Software-control mode lets pdn set this remotely.
const NINO_MODES = [
...
// Profile-first setup: choose a radio profile + how the channel is used +
// how hard the path is. pdn derives the low-level tuneables; "Custom"
// unlocks them. Stored per-port id.
...
// stored profile choices per port
const PORT_SETUP = {
...
// friendly labels + plain-English help for every tuneable (drives tooltips)
const PARAM_HELP = {
...
const AX25_DEFAULTS = { t1Ms:3000, t2Ms:300, t3Ms:180000, n2:8, windowSize:4 };
const KISS_DEFAULTS = { txDelay:300, slotTime:100, txTail:50, persistence:63 };

function persistPct(v){ return Math.round((v/255)*100); }
function pctToPersist(p){ return Math.round((p/100)*255); }

// per-port health rollup (good / degraded / faulted) for Ports + Dashboard
function portHealth(id){
...
// NinoTNC hardware "test button" diagnostic frame — pdn decodes this and
// flashes it on the Ports screen.
const NINO_TEST = {
  portId: "vhf-1",
  receivedAt: "just now",
  firmware: "NinoTNC A3 · fw 2.3.1",
  mode: 4,
  modeLabel: "9600 baud · GFSK · IL2P",
  txdelaySource: "hardware DIP switches",   // vs "software"
  softwareControl: false,                    // <-- not yet in software-control mode
  rssiDbm: -71,
  crcOk: true,
};

// ---- Beacons (ID) ----
const BEACON_DEFAULT = { intervalMinutes: 30, text: "{node}:{call} pdn node — Reading & District ARS" };
const PORT_BEACONS = {
  "vhf-1":  { enabled: true,  intervalMinutes: 30, text: null },                                   // null = use default
...
// ---- NET/ROM + INP3 plain-English guidance ----
// One-liner descriptions for the on/off choices, and tooltip help + units
// for the numeric knobs, so the config tab reads like guidance not jargon.
const NETROM_TOGGLE_HELP = {
...
Object.assign(window, {
  NODE_CONFIG, APPLY_IMPACT, NETROM, NODE_STATUS, PORT_STATUS, SESSIONS,
  FRAME_TYPES, CALLS, PORTS_LIST, PIDS, makeFrame, seedFrames, LINK_STATS,
  LOG_TAIL, USERS, fmtUptime, fmtBytes, hex, randItem,
  KIND_LABEL, KIND_USES_KISS, NINO_MODES, RADIO_PROFILES, CHANNEL_MODES, LINK_DIFFICULTY,
  PORT_SETUP, PARAM_HELP, AX25_DEFAULTS, KISS_DEFAULTS, persistPct, pctToPersist,
  portHealth, NINO_TEST, BEACON_DEFAULT, PORT_BEACONS,
  NETROM_TOGGLE_HELP, NETROM_FIELD_HELP, INP3_FIELD_HELP,
});
