...
- `CarrierSenseTxGate` — CSMA by hardware DCD: an `IAx25Transport` decorator that defers `SendAsync` while the radio reports the channel busy (bounded wait, fail-open), composing with the TNC's own persistence CSMA.
