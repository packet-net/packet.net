...
#pragma warning restore CS8524
    ...
    private void EmitOldIFrame(TransitionContext tx, byte ns, bool selectiveReplay = false)
    {
        ...
        if (selectiveReplay)
        {
            if (!tx.Session.IsOutstanding(ns))
            {
                return;
            }

            if (!tx.Session.SelectivelyRetransmittedSinceAck.Add(ns))
            {
                return;
            }
        }
        if (!tx.Session.SentIFrames.TryGetValue(ns, out var entry))
        {
            return;
        }
