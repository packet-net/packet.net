...
/// <summary>
/// Resolves the node's <see cref="InstallChannel"/> at runtime (the "Channel detection"
/// section of <c>docs/node-self-update-design.md</c>). There is no build stamp: the same
/// <c>.deb</c> can be installed from an apt repo (<c>apt</c>) or <c>dpkg -i</c>'d from a
/// GitHub Release (<c>github</c>) and dpkg records no install method, so the split is
/// decided live from what is actually on the box.
/// </summary>
/// <remarks>
/// <para>Resolution order - every external probe guarded so a non-Debian / dpkg-less /
/// Windows host never throws:</para>
/// <list type="number">
/// <item><b>Does dpkg own the running binary?</b> <c>dpkg-query -S &lt;realpath of
/// /proc/self/exe&gt;</c>. <c>dpkg-query</c> absent (the runner reports not-launched)
/// <em>or</em> the binary not owned by <c>packetnet</c> → <see cref="InstallChannel.Unknown"/>:
/// nothing here manages the files (an unpacked release tarball, a container image, a build
/// from source), so the node never offers to replace them.</item>
...
/// (the conservative fall - worst case we update from Releases, never the reverse).</item>
/// </list>
...
public sealed class RuntimeInstallChannelProvider : IInstallChannelProvider
{
    ...
    /// packaged install (anything else → <see cref="InstallChannel.Unknown"/>).</summary>
    public const string PackageName = "packetnet";

    /// <inheritdoc/>
    public InstallChannel Channel { get; }

    /// <summary>The production provider: shells out via <see cref="SystemProcessRunner"/>
    /// and resolves the running binary from <c>/proc/self/exe</c>.</summary>
    public RuntimeInstallChannelProvider()
        : this(SystemProcessRunner.Instance, runningBinaryPath: null)
    {
    }
    ...
    public RuntimeInstallChannelProvider(IProcessRunner runner, string? runningBinaryPath = null)
    {
        ArgumentNullException.ThrowIfNull(runner);
        Channel = Resolve(runner, runningBinaryPath);
    }

    private static InstallChannel Resolve(IProcessRunner runner, string? runningBinaryPath)
    {
        ...
        // 1. Does dpkg own the *running binary*?
        var binary = runningBinaryPath ?? ResolveRunningBinary();
        if (binary is null || !DpkgOwnsBinary(runner, binary))
        {
            // dpkg-query absent, or not owned → nothing manages us → no self-update.
            return InstallChannel.Unknown;
        }

        // 2. dpkg owns it → can apt upgrade it from a real repo?
