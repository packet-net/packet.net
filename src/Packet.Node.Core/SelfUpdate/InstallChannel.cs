...
{
    /// <summary>Nothing on this host manages the install, or provenance could not be
    /// determined: the running binary has no dpkg record (an unpacked release tarball, a
    /// build from source), or the probes could not run at all. The node refuses to apply an
    /// update - it doesn't know what owns the files, so replacing them is the operator's job.</summary>
    Unknown = 0,
    ...
    /// <c>dpkg -i</c>), never an in-place overwrite. Resolved at runtime: stamp <c>deb</c>,
    /// the binary is dpkg-owned, but <c>apt-cache policy</c> shows no repo origin (or
    /// <c>apt-cache</c> is absent - the conservative fall, since a Releases self-update is
