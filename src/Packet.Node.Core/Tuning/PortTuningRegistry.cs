...
/// <summary>
/// The set of live <see cref="PortTuningSession"/>s, keyed by port id — at most one per port. Split
...
{
    private readonly ConcurrentDictionary<string, PortTuningSession> sessions =
        new(StringComparer.Ordinal);
    private int disposed;

    /// <summary>The live session on a port, or <c>null</c> when none is active.</summary>
    public PortTuningSession? Get(string portId) =>
    ...
    /// </summary>
    public bool TryAdd(PortTuningSession session)
    {
    ...
    /// </summary>
    public bool Remove(PortTuningSession session)
    {
        ArgumentNullException.ThrowIfNull(session);
        return sessions.TryRemove(new KeyValuePair<string, PortTuningSession>(session.PortId, session));
    }
